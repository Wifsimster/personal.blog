...
- **Diagrams must be editorial-grade — not Excel.** No two-bare-rectangles-and-a-dashed-line. Required elements when you draw something:
  ...
  - **Verify every diagram with a real screenshot before shipping** (author's ruling, 2026-09-19). SVG text does not wrap: a legend or label that is a few characters too long for its row does not fail loudly, it silently overflows the canvas — invisible in the source, invisible in `check-post.mjs` (it checks palette/typography/a11y rules, not rendered layout), and easy to miss in a code review. Caught this way on `skills-not-seats`: a two-line legend with swatch + label side by side, where the second label ran past the 640-unit canvas and got clipped by `.diagram-scroll`, unreadable on the published page. Before a diagram ships (new or edited), run the dev/preview server and screenshot the actual rendered post — light and dark, and at a narrow (phone) width, since that's the primary device — rather than eyeballing the SVG source. Read the tool description of whatever browser/screenshot tool is available in the session (Bash + a headless browser, an MCP tool, or the `run` skill) and use it; don't skip this step because the coordinates "look right" on paper.
...
- **Write for fast, low-friction reading** (author's ruling, 2026-09-19, sharpens the 2026-07-19 note above into a hard default): most sentences under ~20 words, one clause. The moment a sentence needs a second comma to hold together, split it. Prefer a full stop over stacking a second idea onto the same sentence with "and" or a semicolon. Short paragraphs — 2-4 sentences, then break. Save the em-dash aside for when it actually earns its place, not as a default connector. This is explicitly about reducing reading friction and time-to-point, not just tone — the same rule that makes a post scannable for a tired reader on a phone also makes it faster for a reader who processes long, clause-stacked sentences worse than short direct ones. Applies to every post going forward, both languages; re-pass existing posts opportunistically when editing them for something else, not as a standalone rewrite project.
