...
<p>Elle a été faite en une session, testée de bout en bout, le 18 septembre. C'est cette bascule-là que je veux décrire — pas le gadget.</p>

<h2>Ce qu'il y a vraiment derrière le mot « domotique »</h2>
...
<p>C'est le point de départ, et il est important. Sur une installation à un protocole, un agent n'a pas grand-chose à apporter. Sur celle-là, chaque idée traverse trois systèmes et deux formats de configuration.</p>
...
<p>Ce que je croyais coûteux dans la domotique, c'était écrire le YAML. C'était faux.</p>
<p>Le vrai coût, c'est de retrouver le contexte. Quelles automatisations touchent cette entité. Laquelle de mes trois listes de jours doit rester synchronisée avec les deux autres. Quel garde-fou, écrit il y a huit mois, va réécraser la commande que je viens d'envoyer.</p>
...
<p>C'est exactement le genre de travail qu'un agent fait mieux qu'un humain fatigué un dimanche soir : lire soixante-quinze automatisations, croiser les références, trouver le conflit.</p>
...
<p>Aucune de ces tâches n'est difficile. Toutes sont exactement assez pénibles pour être repoussées.</p>

<h2>Les idées qui passent enfin sous le seuil</h2>
<p>C'est le vrai changement, et il est invisible sur un relevé d'activité : la liste de ce que j'accepte d'essayer s'est élargie.</p>
...
<p><strong>Et le résultat qui m'a le plus surpris.</strong> Ce même bureau est à 29 °C en pleine nuit, soit trois degrés de plus que le salon, sans aucun soleil. La cause est une baie informatique qui dissipe 331 W en continu. Le volet n'a jamais été le levier. Sans l'analyse, j'aurais passé l'été à régler un barème solaire sur un problème qui n'était pas solaire.</p>
...
<p>Ces quatre chantiers ont un point commun. Ce sont des heures d'analyse qui ne servent qu'une fois. Personne ne fait ça sur une maison. Le calcul économique interdisait de les commencer, pas de les finir.</p>
...
<p>Sur ces prises, la pile réseau et la pile applicative tombent indépendamment. Le juge de paix n'est ni le ping ni le signal : c'est l'annonce <abbr title="Multicast DNS — annonce de services sur le réseau local, sans serveur central">mDNS</abbr> du service, listée depuis l'intérieur du conteneur. Quatre outils différents dans une seule séance, pour aboutir à une conclusion d'une ligne.</p>
<p>Deuxième exemple, plus démonstratif encore. Mes huit ampoules Wi-Fi étaient régulièrement injoignables. L'explication confortable — « elles sont coupées à l'interrupteur mural » — était vraie pour la moitié d'entre elles. Pour les autres, il a fallu interroger la base du contrôleur Wi-Fi : un identifiant de réseau pointant vers un SSID supprimé des mois plus tôt, 86 échecs d'authentification en 24 heures sur une seule ampoule, et trois points d'accès d'appairage diffusés par des lampes que je croyais éteintes.</p>
<p>Troisième : une télécommande infrarouge que je vivais comme « aléatoire ». Mesurée sur trois jours, elle se figeait 318 fois, une fois toutes les neuf minutes en médiane. La porte de l'atelier était donc injoignable un quart du temps. Ce n'était pas aléatoire, c'était fréquent. Elle est aujourd'hui sondée toutes les trente secondes et réveillée automatiquement, sans coupure de courant.</p>
...
<p>La partie dont je suis le plus satisfait ne contient aucun protocole.</p>
...
<p>Elles ont été réécrites d'un coup selon une grille fixe, et c'est cette grille le vrai livrable :</p>
<ul>
...
<p>Voilà le vrai déverrouillage. Quand le coût d'une modification tombe à presque rien, on se met à régler des choses qu'on n'aurait jamais ouvertes. Une maison cesse d'être une installation et devient un objet qu'on ajuste au ressenti, le jour même, par petites touches.</p>
...
<p><strong>Arbitrer l'agent.</strong> Sur ce même volet, il a annoncé « l'étage radio est mort, matériel à remplacer » quatre minutes avant que le module ne revienne tout seul. Un raisonnement propre, une conclusion prématurée. Un agent avance vite, y compris vers une mauvaise réponse, et rien dans son ton ne le signale.</p>
...
<p>Le parc ne convergera pas. Dans cinq ans il y aura toujours un pont propriétaire, une télécommande infrarouge et une ampoule qui refuse son réseau. Ce qui a changé n'est pas la maison : c'est la distance entre une idée et sa version qui marche.</p>`
}
...
<h2>What "home automation" actually contains</h2>
...
<p>That is the starting point and it matters. On a single-protocol setup, an agent has little to add. On this one, every idea crosses three systems and two configuration formats.</p>
...
<p>I used to think the expensive part of home automation was writing the YAML. That was wrong.</p>
<p>The real cost is recovering context. Which automations touch this entity. Which of my three day-lists has to stay in sync with the other two. Which guard, written eight months ago, is about to overwrite the command I just sent.</p>
...
<p>That is exactly the work an agent does better than a tired human on a Sunday night: read seventy-five automations, cross-reference them, find the conflict.</p>
...
<p>None of these tasks is hard. All of them are exactly annoying enough to be postponed.</p>

<h2>The ideas that finally clear the bar</h2>
<p>This is the real change, and it is invisible on any activity log: the list of things I am willing to attempt got wider.</p>
...
<p>These four have something in common. They are hours of analysis that pay off exactly once. Nobody does that to a house. The economics forbade starting them, not finishing them.</p>
...
<p>On these plugs the network stack and the application stack die independently. The arbiter is neither ping nor signal strength. It is the <abbr title="Multicast DNS — service announcement on the local network, with no central server">mDNS</abbr> service announcement, listed from inside the container. Four different tools in one sitting, to reach a one-line conclusion.</p>
<p>Second example, more telling still. My eight Wi-Fi bulbs kept going unreachable. The comfortable explanation — "they are switched off at the wall" — was true for half of them. For the rest it took querying the Wi-Fi controller's own database: a network id pointing at an SSID deleted months earlier, 86 authentication failures in 24 hours on a single bulb, and three pairing access points broadcast by lamps I believed were off.</p>
<p>Third: an infrared remote I experienced as random. Measured over three days, it froze 318 times, a median of once every nine minutes. The workshop door was unreachable a quarter of the time. It was not random, it was frequent. It is now probed every thirty seconds and revived automatically, with no power cycle.</p>
...
<p>The part I am proudest of contains no protocol at all.</p>
...
<p>They were rewritten in one pass against a fixed grid, and that grid is the real deliverable:</p>
<ul>
...
<p>That is the real unlock. When the cost of a change falls to almost nothing, you start tuning things you would never have opened. A house stops being an installation and becomes something you adjust by feel, same-day, in small strokes.</p>
...
<p><strong>Refereeing the agent.</strong> On that same shutter, it announced "the radio stage is dead, replace the hardware" four minutes before the module came back on its own. Clean reasoning, premature conclusion. An agent moves fast, including toward a wrong answer, and nothing in its tone flags the difference.</p>
...
<p>The fleet will not converge. In five years there will still be a proprietary bridge, an infrared remote and a bulb refusing its own network. What changed is not the house. It is the distance between an idea and a version of it that works.</p>`
}
