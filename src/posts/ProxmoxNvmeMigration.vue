...
const frenchContent: PostContent = {
  ...
  html: `<p><strong>TL;DR :</strong> le 14 mai 2026, ma <abbr title="Machine virtuelle, un ordinateur complet simulé par logiciel">VM</abbr> Docker s'est figée. Deux saturations en même temps : la RAM partait en swap thrashing et le thin pool <abbr title="Gestionnaire de volumes logiques Linux, qui découpe les disques indépendamment des partitions physiques">LVM</abbr> de <abbr title="Hyperviseur open source qui fait tourner plusieurs machines virtuelles sur un même serveur">Proxmox</abbr> affichait 97,8 % — <code>dmeventd: Insufficient free space</code>, toutes les I/O bloquées. Le <abbr title="Interface de stockage rapide pour SSD branchés directement sur le bus PCI Express">NVMe</abbr> de 250 Go était plein et le groupe de volumes n'avait plus un octet libre : aucune marge pour réparer. La vraie solution tenait en une phrase — un disque plus gros — et en un problème matériel : la carte mère n'a qu'un seul slot M.2. Voici comment j'ai migré un hôte Proxmox vers un NVMe quatre fois plus grand avec <code>dd</code>, un disque USB en relais et zéro réinstallation. Et la leçon <code>discard=on</code> que je n'ai comprise qu'une semaine plus tard.</p>
<hr>
<p>Un homelab, ça marche très bien jusqu'au jour où ça ne marche plus du tout. Le mien tourne sous Proxmox : deux machines virtuelles — une baie <abbr title="Système d'exploitation de stockage en réseau, tolérant à la panne d'un disque">Unraid</abbr> et une VM Docker qui héberge une soixantaine de conteneurs (Plex, Immich, Paperless, et tout le reste). Pendant des mois, je n'ai pas regardé l'espace disque. C'était l'erreur.</p>
...
const englishContent: PostContent = {
  ...
  html: `<p><strong>TL;DR:</strong> on 14 May 2026, my Docker <abbr title="Virtual machine, a complete computer simulated in software">VM</abbr> froze. Two saturations at once: RAM was thrashing into swap, and <abbr title="Open-source hypervisor running several virtual machines on one server">Proxmox</abbr>'s <abbr title="Linux Logical Volume Manager, carving up disks independently of physical partitions">LVM</abbr> thin pool sat at 97.8% — <code>dmeventd: Insufficient free space</code>, every I/O blocked. The 250 GB <abbr title="Fast storage interface for SSDs attached directly to the PCI Express bus">NVMe</abbr> was full and the volume group had zero free bytes left: no room to repair. The real fix was one sentence — a bigger disk — and one hardware problem: the motherboard has a single M.2 slot. Here is how I migrated a Proxmox host onto an NVMe four times larger with <code>dd</code>, a USB relay drive and zero reinstall. Plus the <code>discard=on</code> lesson I only understood a week later.</p>
<hr>
<p>A homelab works fine right up until the day it doesn't work at all. Mine runs on Proxmox: two virtual machines — an <abbr title="Network storage operating system, tolerant to a single disk failure">Unraid</abbr> array and a Docker VM hosting around sixty containers (Plex, Immich, Paperless, and all the rest). For months, I never looked at the disk space. That was the mistake.</p>
