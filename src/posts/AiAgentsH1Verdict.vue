...
<li><strong>The autonomy infrastructure.</strong> Some fifty architecture decision records, forty-odd CLAUDE.md briefs — one per service — close to forty skills, a couple hundred docs. Nearly one line of Markdown for every five lines of code. That is not bureaucracy; it's the context we inject into agents so they can work a lane without us. We stopped documenting for humans only. We document for fleets.</li>
<li><strong>The tax, made executable.</strong> Over 600 test files, a couple dozen end-to-end specs on the critical journeys, and a <code>task verify</code> gate that returns a PASS/FAIL verdict before any PR exists. The verification tax this series keeps naming is no longer a blog-post concept — it's a binary in the CI.</li>
</ul>
...
<tr><td><strong>AI is an amplifier</strong></td><td>Confirmed</td><td>Throughput up top, quality costs compounding below. <abbr title="Massachusetts Institute of Technology">MIT</abbr> found 95% of <abbr title="Generative AI">GenAI</abbr> pilots delivered no measurable <abbr title="Profit and Loss">P&amp;L</abbr> impact. The tool amplifies the system around it — exactly the claim.</td></tr>
...
<li><strong>End-to-end specs on the critical journeys.</strong> Front-to-back, generated and then maintained. A couple dozen today.</li>
...
<hr>
<p>Depuis mars, mon équipe construit Solstice, une plateforme greenfield partie d'une page blanche. En six mois, on a moins écrit du code qu'on n'a réécrit notre façon d'en produire : un brief par service, des agents qui tiennent chacun leur périmètre, une porte <code>task verify</code> qui rend son verdict avant qu'un humain ne lise le diff. C'est cette méthode que je viens raconter — comment elle s'est mise en place, et ce qu'elle a produit.</p>
...
<li><strong>Mai.</strong> Opus 4.8 le 28. Anthropic affirme qu'il laisse passer quatre fois moins de défauts dans son propre code que 4.7 — un chiffre d'éditeur, certes, mais le choix du chiffre dit tout : plus personne ne vend de la génération, on vend du contrôle. Et avec lui, les « dynamic workflows » de Claude Code : un script rejouable qui répartit le travail sur jusqu'à mille agents par exécution, une quinzaine à la fois, sans noyer le contexte du modèle. Ma <a target="_blank" rel="noopener noreferrer" href="/posts/ai-team-reflex">méthode de panel de personas</a>, en somme, devenue un produit. Le même mois, le protocole <abbr title="Model Context Protocol">MCP</abbr> passe stateless — une flotte d'agents, c'est désormais un problème de load-balancer, plus une démo.</li>
...
<li><strong>Une réécriture complète, en vol.</strong> Un architecture decision record a acté l'abandon de Vue pour React. Quatre mois plus tard : plus de 300 fichiers <code>.tsx</code>, zéro <code>.vue</code> restant. Ce genre de migration, avant, c'était un projet d'un an avec son propre comité de pilotage. Là, c'est un sweep d'agents sur des centaines de fichiers, borné par les tests, fusionné par des humains.</li>
<li><strong>L'infrastructure d'autonomie.</strong> Une cinquantaine d'architecture decision records, une quarantaine de CLAUDE.md — un par service —, une quarantaine de skills, quelque 250 documents. Près d'une ligne de Markdown pour cinq lignes de code. Ce n'est pas de la bureaucratie : c'est le contexte qu'on injecte aux agents pour qu'ils tiennent un périmètre sans nous. On ne documente plus seulement pour les humains. On documente pour les flottes.</li>
<li><strong>La taxe, rendue exécutable.</strong> plus de 600 fichiers de test, une vingtaine de specs <abbr title="End-to-end — de bout en bout">E2E</abbr> sur les parcours critiques, et une porte <code>task verify</code> qui rend un verdict PASS/FAIL avant même qu'une PR existe. La taxe de vérification que cette série n'arrête pas de nommer n'est plus un concept de billet de blog — c'est un binaire dans la CI.</li>
</ul>
...
<blockquote><p>« En 2026, une signature de commit est une chaîne de responsabilité, plus une signature d'auteur. »</p></blockquote>
...
<p><strong>Tout ce qui se décide s'écrit, parce que tout ce qui s'écrit devient du contexte.</strong> Une cinquantaine d'architecture decision records en quatre mois, un brief par service, un glossaire du domaine, une quarantaine de skills pour les gestes répétés. La documentation a changé de nature : ce n'est plus un livrable qui vieillit, c'est le carburant des flottes. Une décision mal écrite, c'est une flotte qui code à côté. Près d'une ligne de Markdown pour cinq lignes de code — le meilleur investissement du projet.</p>
<p><strong>La machine dit non avant que l'humain ne lise.</strong> La porte <code>task verify</code> rend un verdict PASS ou FAIL avant même qu'une PR existe ; la revue humaine ne commence que quand la machine a dit oui. Et l'architecture elle-même sert la méthode : les microservices et les micro-frontends sont des périmètres bornés qu'un agent peut tenir seul, sans marcher sur le travail des autres.</p>
...
<tbody>
<tr><td><strong>Le <abbr title="Software Development Life Cycle — cycle de vie du développement logiciel">SDLC</abbr> est mort</strong></td><td>Tient, amendé</td><td>La cérémonie est morte ; une porte de vérification est née. Le processus n'a pas rétréci — son centre de gravité est passé de l'écriture à la revue.</td></tr>
<tr><td><strong>L'IA est un amplificateur</strong></td><td>Confirmé</td><td>Débit en hausse en surface, coûts qualité qui se composent en dessous. Le <abbr title="Massachusetts Institute of Technology">MIT</abbr> : 95 % des pilotes d'IA générative sans impact mesurable sur le <abbr title="Profit and Loss — compte de résultat">P&amp;L</abbr>. L'outil amplifie le système autour — exactement le pari.</td></tr>
...
<ol>
<li><strong>La PR de périmètre borné, de bout en bout.</strong> Ticket → branche → code → tests → verdict <code>task verify</code> → PR ouverte. L'humain n'apparaît qu'à la revue. Condition non négociable : le périmètre a son CLAUDE.md, ses skills, et la porte rend PASS.</li>
...
<li><strong>Les specs E2E sur les parcours critiques.</strong> Du front jusqu'au back, générées puis maintenues. Une vingtaine aujourd'hui.</li>
...
<blockquote><p>« L'autonomie ne se décrète pas au niveau du modèle. Elle s'équipe, périmètre par périmètre — un brief, des skills, une porte qui sait dire non. »</p></blockquote>
