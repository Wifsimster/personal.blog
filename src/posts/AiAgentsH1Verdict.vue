...
<hr>
<p>Since March, my team has been building Solstice, a greenfield platform started from a blank page. In four months we wrote less code than we rewrote the way we produce it: a brief per service, agents that each hold their own lane, a <code>task verify</code> gate that returns its verdict before a human reads the diff. That method is what I came to write down — how we put it in place, and what it produced.</p>
...
<hr>
<p>Depuis mars, mon équipe construit Solstice, une plateforme greenfield partie d'une page blanche. En quatre mois, on a moins écrit du code qu'on n'a réécrit notre façon d'en produire : un brief par service, des agents qui tiennent chacun leur périmètre, une porte <code>task verify</code> qui rend son verdict avant qu'un humain ne lise le diff. C'est cette méthode que je viens raconter — comment elle s'est mise en place, et ce qu'elle a produit.</p>
