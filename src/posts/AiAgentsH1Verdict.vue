...
<p><strong>Deciding whether the code is safe to merge.</strong> That's the tax, and it didn't get cheaper — it got bigger, and it moved to the front of the pipeline. A 32.7% acceptance rate is a human saying "no" thousands of times a day. On our repository, the verification gate now says "no" before a human even reads the diff — but the merge click stays human, every single time. That judgment is the job now.</p>
...
<hr>
<p>Depuis mars, mon équipe construit Solstice, une plateforme greenfield partie d'une page blanche. En quatre mois, on a moins écrit du code qu'on n'a réécrit notre façon d'en produire : un brief par service, des agents qui tiennent chacun leur périmètre, une porte de vérification qui rend son verdict avant qu'un humain ne lise le diff. C'est cette méthode que je viens raconter — comment elle s'est mise en place, et ce qu'elle a produit.</p>
...
<li><strong>La taxe, rendue exécutable.</strong> plus de 600 fichiers de test, une vingtaine de specs <abbr title="End-to-end — de bout en bout">E2E</abbr> sur les parcours critiques, et une porte de vérification qui rend un verdict PASS/FAIL avant même qu'une PR existe. La taxe de vérification que cette série n'arrête pas de nommer n'est plus un concept de billet de blog — c'est un binaire dans la CI.</li>
</ul>
...
<p><strong>La machine dit non avant que l'humain ne lise.</strong> La porte de vérification rend un verdict PASS ou FAIL avant même qu'une PR existe ; la revue humaine ne commence que quand la machine a dit oui. Et l'architecture elle-même sert la méthode : les microservices et les micro-frontends sont des périmètres bornés qu'un agent peut tenir seul, sans marcher sur le travail des autres.</p>
...
<ol>
<li><strong>La PR de périmètre borné, de bout en bout.</strong> Ticket → branche → code → tests → verdict de vérification → PR ouverte. L'humain n'apparaît qu'à la revue. Condition non négociable : le périmètre a son CLAUDE.md, ses skills, et la porte donne son feu vert.</li>
