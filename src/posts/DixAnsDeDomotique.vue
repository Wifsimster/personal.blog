...
const frenchContent: PostContent = {
  ...
  html: `<div class="post-tldr"><p><strong>TL;DR</strong> Mon dernier billet de domotique date d'avril 2016 : une sonde BMP180 sur un <abbr title="Microcontrôleur Wi-Fi bon marché, très répandu chez les bricoleurs">ESP8266</abbr>, remontée dans Jeedom. Dix ans plus tard la maison compte 87 appareils sur neuf écosystèmes, 929 entités et 75 automatisations. Ce billet ouvre une série : il pose l'inventaire, raconte comment on en arrive là un week-end à la fois, et mesure ce qui a changé depuis que je travaille avec un agent sur cette installation. Deux exemples portent la différence. Deux enceintes Sonos livrées un mercredi, intégrées et câblées à treize annonces de routine le lendemain. Et un visiophone dont l'appel ne sortait par aucune intégration, récupéré via un pont maison de 225 lignes.</p></div>
<hr>
<p>En avril 2016, j'ai publié ici un montage de capteur de luminosité sur ESP8266, remonté dans Jeedom. C'est le dernier billet de domotique de ce blog.</p>
<p>Pas le dernier montage. Entre-temps il y a eu une maison de 1970 achetée en 2019, une pièce rénovée par an jusqu'en 2025, et une installation qui a grossi sans que j'en écrive une ligne. Cette série répare ça.</p>
<p>Ce premier épisode est un état des lieux. Ce qu'il y a dans la maison, comment ça s'est empilé, et ce que douze mois de travail avec un agent ont changé à un chantier que je menais seul depuis dix ans.</p>

<h2>L'époque du fer à souder</h2>
<p>Le premier billet domotique de ce blog date de février 2014 : piloter des prises Chacon depuis un Raspberry Pi. Ensuite un Ambilight en WS2801, puis un second en WS2812b piloté par un Teensy. En 2016, six billets d'affilée sur l'ESP8266 — le flashage, la communication sans fil, un afficheur I2C, une sonde de température, un capteur de luminosité, et un distributeur de croquettes.</p>
...
<p>Deux Sonos Era 100 livrées le 16 septembre. Le lendemain elles étaient dans Home Assistant, nommées par pièce, rattachées au salon et à la chambre, et branchées sur treize annonces de routine.</p>
...
<p>Le jugement reste aussi du côté humain, et pas seulement pour les décisions techniques. La coupure de télévision de 19 h 30 n'est pas une optimisation de consommation : c'est un repère de fin de journée pour un enfant. J'ai proposé deux fois de rationaliser son calendrier. Les deux fois, la bonne réponse était non.</p>
...
<li><strong>Le prix d'essayer</strong> — pourquoi le gain se mesure sur les chantiers qu'on ne commençait pas, pas sur ceux qu'on faisait déjà.</li>
<li><strong>Le voyant vert ne prouve rien</strong> — neuf protocoles, neuf définitions de « en ligne », et les pannes qui restent invisibles trois jours.</li>
...
const englishContent: PostContent = {
  ...
  html: `<div class="post-tldr"><p><strong>TL;DR</strong> My last home automation post here is dated April 2016: a BMP180 probe on an <abbr title="A cheap Wi-Fi microcontroller, widely used by hobbyists">ESP8266</abbr>, reporting into Jeedom. Ten years on, the house holds 87 devices across nine ecosystems, 929 entities and 75 automations. This post opens a series. It sets out the inventory, tells how you get there one weekend at a time, and measures what changed once I started working on this installation with an agent. Two examples carry the difference. Two Sonos speakers delivered on a Wednesday, integrated and wired into thirteen routine announcements the next day. And a door entry panel whose call event reached no integration at all, recovered through a 225-line bridge written from scratch.</p></div>
<hr>
<p>In April 2016 I published a light-sensor build on an ESP8266, reporting into Jeedom. It is the last home automation post on this blog.</p>
<p>Not the last build. In between there was a 1970s house bought in 2019, one room renovated a year until 2025, and an installation that grew without a single line written about it here. This series fixes that.</p>
<p>This first episode is a survey. What is in the house, how it piled up, and what twelve months of working with an agent changed about a project I had been running alone for ten years.</p>

<h2>The soldering-iron years</h2>
<p>The first home automation post on this blog is from February 2014: driving mains sockets from a Raspberry Pi. Then an Ambilight on WS2801, then a second one on WS2812b driven by a Teensy. In 2016, six posts in a row on the ESP8266 — flashing it, wireless comms, an I2C display, a temperature probe, a light sensor, and a cat food dispenser.</p>
...
<p>Two Sonos Era 100 delivered on 16 September. The next day they were in Home Assistant, named per room, attached to the living room and the bedroom, and wired into thirteen routine announcements.</p>
...
<p>Judgment stays on the human side too, and not only for technical calls. The 7:30 p.m. television cut is not an energy optimisation: it is an end-of-day landmark for a child. I twice proposed rationalising its calendar. Both times the right answer was no.</p>
...
<li><strong>The cost of trying</strong> — why the win shows up on the projects you never started, not on the ones you already did.</li>
<li><strong>Green lights prove nothing</strong> — nine protocols, nine definitions of "online", and the outages that stay invisible for three days.</li>
